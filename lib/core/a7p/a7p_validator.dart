...
class A7pValidator {
  ...
  static void _validateProfile(Profile p, List<A7pFieldError> errors) {
    ...
    // ── Cartridge ─────────────────────────────────────────────────────────────
    _checkRange(errors, 'c_muzzle_velocity',    p.cMuzzleVelocity,    10,   30000); // m/s ×10
    _checkRange(errors, 'c_zero_temperature',   p.cZeroTemperature,   -100, 100);   // °C
    _checkRange(errors, 'c_t_coeff',            p.cTCoeff,            0,    1000);  // %/15°C ×1000
    _checkRange(errors, 'c_zero_p_temperature', p.cZeroPTemperature,  -100, 100);   // °C

    // ── Zero conditions ───────────────────────────────────────────────────────
    _checkRange(errors, 'c_zero_air_temperature', p.cZeroAirTemperature, -100, 100);  // °C
    _checkRange(errors, 'c_zero_air_pressure',    p.cZeroAirPressure,    3000, 15000); // hPa ×10
    ...
    if (dists.isEmpty || dists.length > 200) {
      errors.add(A7pFieldError(
          'distances', 'must have 1–200 items, got ${dists.length}'));
    } else {
      for (var i = 0; i < dists.length; i++) {
        _checkRange(errors, 'distances[$i]', dists[i], 100, 300000);
      }
      // zero_distance_idx must be a valid index
      _checkRange(errors, 'c_zero_distance_idx', p.cZeroDistanceIdx, 0, dists.length - 1);
    }

    // ── Switches ──────────────────────────────────────────────────────────────
    if (p.switches.length < 4) {
      errors.add(A7pFieldError(
          'switches', 'must have at least 4 items, got ${p.switches.length}'));
    } else {
  ...
  static void _validateCoefRows(
      List<CoefRow> rows, GType bcType, List<A7pFieldError> errors) {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      if (r.mv != 0) {
        if (!seenMv.add(r.mv)) {
          errors.add(A7pFieldError(
              'coef_rows[$i].mv', 'duplicate mv value ${r.mv}'));
        }
      }
    }
  }
  ...
  static void _requireString(
      List<A7pFieldError> errors, String field, String value,
      {required int maxLen}) {
    if (value.isEmpty) {
      errors.add(A7pFieldError(field, 'required'));
    } else if (value.length > maxLen) {
      errors.add(A7pFieldError(
          field, 'max $maxLen chars, got ${value.length}'));
    }
  }

  static void _checkRange(
      List<A7pFieldError> errors, String field, int value, int min, int max) {
